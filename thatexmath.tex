...
% maybe the font bbm has something interesting too
\font\tenbbb=msbm10
\font\sevenbbb=msbm7
\font\fivebbb=msbm5
...
\def\namedvar#1{\mathord{\mathit{#1}}}% for variables
\def\namedfpf#1{\mathord{\mathui{#1}}}% for functional programming functions
...
\def\namedop#1{\mathop{\operatorname{#1}}\nolimits}
\def\namedoplims#1{\mathop{\operatornamewithlimits{#1}}}
\def\cal{\Cal}
%%}}}
...
\def\ints{{\bbb Z}}
\def\reals{{\bbb R}}
\def\xreals{{\overline{\reals}}}
\def\complex{{\bbb C}}
\def\bools{{\bbb B}}
\def\polys#1#2{#1\bracket{#2}} % #1: field ; #2: variable
% subsets of reals
\def\natreals{\reals_\nats}
\def\intreals{\reals_\ints}
\def\ratreals{\reals_\rats}
\def\algreals{\reals_\algs}
% subsets of rats
\def\natrats{\rats_\nats}
\def\intrats{\rats_\ints}
% subsets of ints
\def\natints{\ints\nats}
...
\def\Complexes{{\mathord{\mathbf C}}} 
\def\Ze{{\mathsf 0}}%
\def\Su{{\mathsf S}}%
%%}}}
...
\csgroup\def{#1PPPP}##1##2##3##4{\csname #1\endcsname\funparen{##1,##2,##3,##4}}%
}
% \DefApp is similar but defines a \fa-as-application version
% and also versions that are space-delimited
% and also "columnded" versions to be used inside fundefs
\def\DefApp#1 {%
\csgroup\def{#1A}##1{\csgroup\fA                {#1} {##1}}%
\csgroup\def{#1P}##1{\csgroup\fP                {#1} {##1}}%
\csgroup\def{#1AA}##1##2{\csgroup\fAA           {#1} {##1}{##2}}%
\csgroup\def{#1AP}##1##2{\csgroup\fAP           {#1} {##1}{##2}}%
\csgroup\def{#1PA}##1##2{\csgroup\fPA           {#1} {##1}{##2}}%
\csgroup\def{#1PP}##1##2{\csgroup\fPP           {#1} {##1}{##2}}%
\csgroup\def{#1AAA}##1##2##3{\csgroup\fAAA      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1AAP}##1##2##3{\csgroup\fAAP      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1APA}##1##2##3{\csgroup\fAPA      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1APP}##1##2##3{\csgroup\fAPP      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1PAA}##1##2##3{\csgroup\fPAA      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1PAP}##1##2##3{\csgroup\fPAP      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1PPA}##1##2##3{\csgroup\fPPA      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1PPP}##1##2##3{\csgroup\fPPP      {#1} {##1}{##2}{##3}}%
\csgroup\def{#1AAAA}##1##2##3##4{\csgroup\fAAAA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1AAAP}##1##2##3##4{\csgroup\fAAAP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1AAPA}##1##2##3##4{\csgroup\fAAPA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1AAPP}##1##2##3##4{\csgroup\fAAPP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1APAA}##1##2##3##4{\csgroup\fAPAA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1APAP}##1##2##3##4{\csgroup\fAPAP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1APPA}##1##2##3##4{\csgroup\fAPPA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1APPP}##1##2##3##4{\csgroup\fAPPP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PAAA}##1##2##3##4{\csgroup\fPAAA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PAAP}##1##2##3##4{\csgroup\fPAAP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PAPA}##1##2##3##4{\csgroup\fPAPA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PAPP}##1##2##3##4{\csgroup\fPAPP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PPAA}##1##2##3##4{\csgroup\fPPAA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PPAP}##1##2##3##4{\csgroup\fPPAP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PPPA}##1##2##3##4{\csgroup\fPPPA {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1PPPP}##1##2##3##4{\csgroup\fPPPP {#1} {##1}{##2}{##3}{##4}}%
\csgroup\def{#1S}##1 {\csname #1A\endcsname{##1}}%
\csgroup\def{#1SS}##1 ##2 {\csname #1AA\endcsname{##1}{##2}}%
\csgroup\def{#1SSS}##1 ##2 ##3 {\csname #1AAA\endcsname{##1}{##2}{##3}}%
\csgroup\def{#1SSSS}##1 ##2 ##3 ##4 {\csname #1AAAA\endcsname{##1}{##2}{##3}{##4}}%
\csgroup\def{#1C}##1 {\csname #1\endcsname&{##1}}%
\csgroup\def{#1CC}##1 ##2 {\csname #1\endcsname&{##1}&{##2}}%
\csgroup\def{#1CCC}##1 ##2 ##3 {\csname #1\endcsname&{##1}&{##2}&{##3}}%
\csgroup\def{#1CCCC}##1 ##2 ##3 ##4 {\csname #1\endcsname&{##1}&{##2}&{##3}&{##4}}%
}
...
\def\DefTYPE#1 #2 {\csgroup\def{#1}{{\namedtype{#2}}}\DefPar #1 \DefApp #1 }% Par/App needed for type constructors like List and Maybe
\def\DefVAR#1 #2  {\csgroup\def{#1}{{\namedvar{#2}}}}
...
\def\doublecolon{\mathbin{:\mkern-1mu:}}
\def\rrarr{\rightrightarrows}% ⇉
\def\rlarr{\rightleftarrows}% ⇄
\def\lrarr{\leftrightarrows}% ⇆
\def\llarr{\leftleftarrows}% ⇇
\def\turnstile{\vdash}% ⊢
%%}}}
...
\def\bsystemed#1{\flexbrace{\systemed{#1}}}
...
\def\bmodsystemed#1{\flexbrace{\alignedat3\relax #1\endalignedat}}
...
\def\toverbracebig#1#2{\overbrace{#1}^{\dsize\text{\tenrm #2}}}% XXX: default \text can't display some non-ascii
\def\moverbrace#1#2{\overbrace{#1}^{{#2}}}
\def\moverbracebig#1#2{\overbrace{#1}^{{\dsize #2}}}% big (legible) version
\def\tunderbrace#1#2{\underbrace{#1}_{\text{\eightrm #2}}}% XXX: default \text can't display some non-ascii
\def\tunderbracebig#1#2{\underbrace{#1}_{\dsize\text{\tenrm #2}}}% XXX: default \text can't display some non-ascii
\def\munderbrace#1#2{\underbrace{#1}_{{#2}}}
\def\munderbracebig#1#2{\underbrace{#1}_{{\dsize #2}}}% big (legible) version
\let\tobrace=\toverbrace
\let\tubrace=\tunderbrace
\let\mobrace=\moverbrace
\let\mubrace=\munderbrace
\let\tobraceb=\toverbracebig
\let\tubraceb=\tunderbracebig
\let\mobraceb=\moverbracebig
\let\mubraceb=\munderbracebig
\def\byobrace#1#2{\mathrel{\tobrace {#1} {\clap{\explanationstylize{(#2)}}}}}
\def\byubrace#1#2{\mathrel{\tubrace {#1} {\clap{\explanationstylize{(#2)}}}}}
\def\shell#1\endshell{\quote{\let\\=\newline\tentt #1}\endquote}% TODO: make a real shell macro
\def\cased#1{{\let\\=\cr\knuthcases{#1}}}
\def\cases#1\endcases{\cased{#1}}
...
\def\itext#1\enditext{\intertext{#1}}


% funcompute and poemcompute used to have code factored out
% but having independent implementations makes it easier to
% change their behaviour in the future as needed

% \funcompute (Bird atributes this style to Wim Feijen)
\def\funcomputedPRIME#1{%
\def\does##1{\\\llap{{}##1{}\ \ }{}\quad}%
\let\by=\mathby
\matrix\format\l\\
#1
\endmatrix
}
\let\funcomputed=\funcomputedPRIME
\def\funcomputesed#1{&\funcomputedPRIME{#1}}% for use in funcomputes only
\def\funcompute#1\endfuncompute{$$
\spreadmatrixcr
\gather\fixmcount
\funcomputed{#1}
\endgather\saveeqnumber
$$}
\def\funcomputes#1 #2\endfuncomputes{$$
\let\funcomputed=\funcomputesed
\spreadmatrixcr
\xalignat#1\relax\fixmcount
#2
\endxalignat\saveeqnumber
$$}

% \poemcompute
\def\poemcomputepoems#1{%
\matrix\format\ \ {}\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\\
#1
\endmatrix
}
\def\poemcomputePRIME #1\\#2\endpoemcomputePRIME{% #2 is discarded to consume unwanted space
{%
\def\by##1{&\ \ \mathby{##1}}%
\mathcoled {%
&#1 \\
...
\def\poemcomputed#1{\poemcomputePRIME #1\endpoemcomputePRIME}
\def\poemcomputesed#1{&\poemcomputePRIME #1\endpoemcomputePRIME}% repetirion of code necessary here
\def\poemcompute#1\endpoemcompute{$$\spreadmatrixcr\poemcomputePRIME #1\endpoemcomputePRIME$$}
\def\poemcomputes#1 #2\endpoemcomputes{$$
\let\poemcomputed=\poemcomputesed
\spreadmatrixcr
\xalignat#1\relax\fixmcount #2\endxalignat\saveeqnumber
$$}
...
\def\reltag#1#2{\mathrel{\buildrel {#2} \over {#1}}}
...
\let\knuthncong=\ncong
\let\knuthdiv=\div
\let\knuthapprox=\approx
\undefine\approx
\def\approx{\knuthapprox}
\def\cong{\equiv}
\undefine\ncong
\def\ncong{\mathrel{\not\equiv}}
...
\def\defiffsymbol{\mathord{\buildrel{\deftag} \over {\Longleftrightarrow}}}
\def\impliesbecause#1{\mathrel{{\buildrel{\text{\explanationstylize{\sixrm #1}}} \over {\implies}}}}
\def\implieslabel#1{\labelrel{\implies}{#1}}
\def\relby#1#2{\mathrel{{\buildrel{\text{\clap{\explanationstylize{\sixrm (#2)}}}} \over {#1}}}}
...
\def\defintiffsymbol{\mathord{\buildrel{\deftag} \over {\intiffsymbol}}}
...
%%{{{ logic formulae 
% quantifier symbols and defaults
\let\knuthexists=\exists
\let\knuthforall=\forall
\def\uniquesym{\mathord{\knuthexists!}}
\let\existssym=\knuthexists
\let\forallsym=\knuthforall
% defaults
\let\forall=\forallsym
\let\exists=\existssym
\let\unique=\uniquesym
% /quantifier symbols
\def\univ{U}
...
\def\medquantified#1{\medbracket{\thinspace#1\thinspace}}% the point is that these won't flex to fit content
\def\bigquantified#1{\bigbracket{\thinspace#1\thinspace}}
\def\quantifiedt#1{\quantified{\textrm{#1}}}
\def\medquantifiedt#1{\medquantified{\textrm{#1}}}
\def\bigquantifiedt#1{\bigquantified{\textrm{#1}}}
% quantifiers
\def\quantifierabbr#1#2#3{\paren{\/#1 #2}\quantified{#3}}
\def\quantifierabbrt#1#2#3{\paren{\/#1 #2}\quantifiedt{#3}}
\def\quantifiertabbr#1#2#3{\paren{\/#1 \textrm{#2}}\quantified{#3}}
\def\quantifiertabbrt#1#2#3{\paren{\/#1 \textrm{#2}}\quantifiedt{#3}}
\def\bigquantifierabbr#1#2#3{\paren{\/#1 #2}\bigquantified{#3}}
\def\bigquantifierabbrt#1#2#3{\paren{\/#1 #2}\bigquantifiedt{#3}}
\def\bigquantifiertabbr#1#2#3{\paren{\/#1 \text{#2}}\bigquantified{#3}}
\def\bigquantifiertabbrt#1#2#3{\paren{\/#1 \text{#2}}\bigquantifiedt{#3}}
% columnized, for use in \mathcol
\def\cforall#1#2{\pforall{#1}&\quantified{#2}}
\def\cexists#1#2{\pexists{#1}&\quantified{#2}}
\def\cforallt#1#2{\pforall{#1}&\quantifiedt{#2}}
\def\cexistst#1#2{\pexists{#1}&\quantifiedt{#2}}
% TODO: maybe the \lforall that follows should be called \forall
...
\def\tforallt{\quantifiertabbrt{\forall}}
\def\texistst{\quantifiertabbrt{\exists}}
\def\tuniquet{\quantifiertabbrt{\unique}}
...
\let\knuthlim=\lim
\let\knuthliminf=\liminf
\let\knuthlimsup=\limsup
...
\def\inhabsubset{\mathrel{\subset\mkern-13mu\raise .18ex\hbox{$\cdot$}\,}}
\def\inhabsupset{\mathrel{\supset\mkern-16mu\raise .18ex\hbox{${\pmb\cdot}$}\,\,}}
...
\let\isubset=\inhabsubset
\let\isupset=\inhabsupset
\let\contains=\supset
...
\def\Interl{\Inter\nolimits} % same line
\def\liml{\knuthlim\nolimits} % same line
\def\lim{\knuthlim\nolimits} % lim is sameline by default
\let\limit=\lim
\let\liminflimits=\knuthliminf
\let\limsuplimits=\knuthlimsup
\edef\liminf{\liminf\nolimits}
\edef\limsup{\limsup\nolimits}
\let\minus=\setminus
...
\def\set#1{\brace{#1}}
\def\fsset#1{\{#1\}}% fixed-size set (alt: use \noflexmed)
\def\bigset#1{{\mathopen{\big\{}{#1}\mathclose{\big\}}}}% forced big-size set (alt: use \noflexbig)
\def\tupa#1{\left\langle#1\right\rangle}
\def\tupp#1{\left(#1\right)}
\let\tup=\tupa
\def\binprojectionsymbol{\mathbin{\pmb.}}
\let\bincoprojectionsymbol=\binprojectionsymbol
\def\project#1.#2{{#1}\binprojectionsymbol{\mathsfdc{#2}}}
\def\coproject#1.#2{{\mathsfdc{#1}}\bincoprojectionsymbol{#2}}
\let\prodout=\project
\let\sumin=\coproject
...
\def\FF{\mathord{\ttolkeyword{f}\mkern-2mu\ttolkeyword{f}}}
\def\TT{\mathord{\ttolkeyword{t}\mkern-3mu\ttolkeyword{t}}}
\def\boom{\ttolkeyword{boom}}
\DefFun fst
\DefFun snd
\DefFun swap
\DefFun outl
...
\DefFun compose
% parens
\def\flexparen#1{{\left({#1}\right)}}
\def\lparen#1{{\left({#1}_{\vphantom ,}\right)}}% smart-large parens XXX
\def\medparen#1{{\mathopen{(}{#1}\mathclose{)}}}% forced normal-size parens
...
\def\funparen#1{\paren{#1}}
% brackets
\def\flexbracket#1{{\left[{#1}\right]}}
\def\medbracket#1{{\mathopen{[}{#1}\mathclose{]}}}
\def\bigbracket#1{{\mathopen{\big[}{#1}\mathclose{\big]}}}
\def\Bigbracket#1{{\mathopen{\Big[}{#1}\mathclose{\Big]}}}
\def\biggbracket#1{{\mathopen{\bigg[}{#1}\mathclose{\bigg]}}}
% braces (curly)
\def\flexbrace#1{\left\{#1\right\}}
\def\medbrace#1{\mathopen{\{}#1\mathclose{\}}}
\def\bigbrace#1{\mathopen{\big\{}#1\mathclose{\big\}}}
\def\Bigbrace#1{\mathopen{\Big\{}#1\mathclose{\Big\}}}
\def\biggbrace#1{\mathopen{\bigg\{}#1\mathclose{\bigg\}}}
% flexing contexters
\def\flex      {\let\brace=\flexbrace\let\paren=\flexparen\let\bracket=\flexbracket}
\def\noflexmed {\let\brace=\medbrace \let\paren=\medparen \let\bracket=\medbracket }
\def\noflexbig {\let\brace=\bigbrace \let\paren=\bigparen \let\bracket=\bigbracket }
\def\noflexBig {\let\brace=\Bigbrace \let\paren=\Bigparen \let\bracket=\Bigbracket }
\def\noflexbigg{\let\brace=\biggbrace\let\paren=\biggparen\let\bracket=\biggbracket}
\let\noflex=\noflexbig
% flex by default (birrrl)
\flex
...
\let\seq=\sequence
\def\sequenceset#1#2{{\set{#1}_{#2}}}
\let\seqset=\sequenceset
\def\seqn#1#2{\sequence {{#1}_{#2}} {#2}}
\def\sequencefromto#1#2#3{{\paren{#1}_{#2}^{#3}}}
\def\seqft#1#2#3#4{\sequencefromto {#1} {{#2}={#3}} {#4}} % from to
\def\seqnset#1#2{\set {{#1}_{#2}}_{#2}}
\let\seqst=\famst
\def\ssetp#1#2{({#1}\mathrel{;}{#2})}% structured sets
...
\def\ifthenelsecr#1#2#3{\topaligned&\mathbf{if}~#1\\&\phantom{\mathbf{if}~}\mathbf{then}~#2\\&\phantom{\mathbf{if}~}\mathbf{else}~#3\endtopaligned}
\def\IF#1\THEN#2\ELSE#3\FI{\ifthenelse{#1}{#2}{#3}}
\def\IFCR#1\THEN#2\ELSE#3\RCFI{\ifthenelsecr{#1}{#2}{#3}}
\def\PIF#1\THEN#2\ELSE#3\FIP{\pifthenelse{#1}{#2}{#3}}
\def\letin#1#2{\mathbf{let}~#1~\mathbf{in}~#2}
\def\letcrin#1#2{\topaligned \mathbf{let}~&#1 \\\mathbf{in}~&#2 \endtopaligned}% let..in.. in separate lines
\def\pletin#1#2{\paren{\letin{#1}{#2}}}
\def\where#1\endwhere{\quad \mathbf{where}~\mathrlap{\topaligned\funioed{#1}\endtopaligned}}
\def\nowhere{\quad \phantom{\mathbf{where}}}% use when displaying two fundefs side by side and only one has an extra where line
\def\whereblock#1\endwhereblock{\quad \mathbf{where}~\left[\mathrlap{\topaligned\funioed{#1}\endtopaligned}\right.}
\def\wherecr#1\endwherecr{\quad\mathbf{where} \\\qquad\mathrlap{\funioed{#1}}}
\def\guards#1\endguards{\quad\left|\;\mathrlap{\def\otherwise{\mathrm{otherwise}}\funioed{#1}}\right.}
\def\LET#1\IN#2\TEL{\letcrin{#1}{#2}}
\def\PLET#1\IN#2\TELP{\pletin{#1}{#2}}
\def\mapsio#1#2{\paren{#1\mapsto#2}}% maps input (#1) to output (#2)
\def\binderdot{.}
\def\lamsym{\mathsf{λ}}
\def\lamhead#1{{\lamsym}{#1}}% just the head without any body
\def\lam#1#2{\lamhead{#1}\,\binderdot\,{#2}}
\def\lamt#1#2{\lam{#1}{\text{#2}}}
\def\lamp#1#2{\lam{#1}{\paren{#2}}}% parenthesize body
\def\plam#1#2{\medparen{\lam{#1}{#2}}}% parenthesize whole term
\def\plamt#1#2{\medparen{\lamt{#1}{#2}}}
\def\plamp#1#2{\medparen{\lamp{#1}{#2}}}
\def\lapp{\nobreak\;\nobreak}% lambda term application
\def\fapp{\mathord{\;}}% function application
...
\let\la=\lapp
\let\lap=\lappp
\def\descrsym{\mathsf{ι}}
\def\descrhead#1{{\descrsym}{#1}}% just the head without any body
\def\descr#1#2{\descrhead{#1}\,.\,{#2}}
...
%%{{{ lambda calculus 
\def\stepsym{\triangleright}
\def\Llam#1#2{\lam{#1}{#2}}
\def\Lmac#1{\underline{\mathtt{#1}}}
\def\Lfinto{\rightarrow\mathrel{\kern-.8em}\rightarrow}
\def\Lnum#1{{\underline{#1}}}
\def\xstep[#1]{\mathrel{\;\stepsym{\kern-.4em{}_{#1}\;}}}
\def\tstep[#1]{\mathrel{\;\stepsym{\mkern-4mu{}_{\raise-.2ex\textrmtiny{#1}}\;}}}
\def\lamstep{\tstep[λ]}
\def\juststep{\xstep[\phantom{\beta}]}
\def\alphastep{\tstep[α]}
\def\betastep{\tstep[β]}
\def\etastep{\tstep[η]}
\def\calcstep{\tstep[c]}
...
\let\stepsto=\juststep
\def\substitute#1#2#3{{#1}\bracket{{#2} \asseq {#3}}}
\def\subst#1[#2 := #3]{\substitute{#1}{#2}{#3}}
%%}}}
...
\DefType Konst
\DefTYPE FinSum  Sum
\DefTYPE FinProd Prod
\def\infers{\mathrel{\turnstile}}
\def\lnil{\mathord{[\,]}}% Nil list sugar: []
\def\lcons{\mathbin{\doublecolon}}% Cons list sugar: :
...
\let\fpf=\namedfpf
\let\var=\namedvar
\def\fsec#1#2{(\fA {\mathord{#1}} {\mathord{#2}})}% sections of binary ops: \fsec + 1; \fsec 1 +; …

% fun applicators:
% fA atomic application
% fP parenthesized atomic application
\def\fA          #1#2{{#1}\fa  {#2}}
\def\fP          #1#2{{#1}\fap {#2}}
\def\fAA       #1#2#3{{#1}\fa  {#2}\fa  {#3}}
\def\fAP       #1#2#3{{#1}\fa  {#2}\fap {#3}}
\def\fPA       #1#2#3{{#1}\fap {#2}\fa  {#3}}
\def\fPP       #1#2#3{{#1}\fap {#2}\fap {#3}}
\def\fAAA    #1#2#3#4{{#1}\fa  {#2}\fa  {#3}\fa  {#4}}
\def\fAAP    #1#2#3#4{{#1}\fa  {#2}\fa  {#3}\fap {#4}}
\def\fAPA    #1#2#3#4{{#1}\fa  {#2}\fap {#3}\fa  {#4}}
\def\fAPP    #1#2#3#4{{#1}\fa  {#2}\fap {#3}\fap {#4}}
\def\fPAA    #1#2#3#4{{#1}\fap {#2}\fa  {#3}\fa  {#4}}
\def\fPAP    #1#2#3#4{{#1}\fap {#2}\fa  {#3}\fap {#4}}
\def\fPPA    #1#2#3#4{{#1}\fap {#2}\fap {#3}\fa  {#4}}
\def\fPPP    #1#2#3#4{{#1}\fap {#2}\fap {#3}\fap {#4}}
\def\fAAAA #1#2#3#4#5{{#1}\fa  {#2}\fa  {#3}\fa  {#4}\fa  {#5}}
\def\fAAAP #1#2#3#4#5{{#1}\fa  {#2}\fa  {#3}\fa  {#4}\fap {#5}}
\def\fAAPA #1#2#3#4#5{{#1}\fa  {#2}\fa  {#3}\fap {#4}\fa  {#5}}
\def\fAAPP #1#2#3#4#5{{#1}\fa  {#2}\fap {#3}\fap {#4}\fap {#5}}
\def\fAPAA #1#2#3#4#5{{#1}\fa  {#2}\fap {#3}\fa  {#4}\fa  {#5}}
\def\fAPAP #1#2#3#4#5{{#1}\fa  {#2}\fap {#3}\fa  {#4}\fap {#5}}
\def\fAPPA #1#2#3#4#5{{#1}\fa  {#2}\fap {#3}\fap {#4}\fa  {#5}}
\def\fAPPP #1#2#3#4#5{{#1}\fa  {#2}\fap {#3}\fap {#4}\fap {#5}}
\def\fPAAA #1#2#3#4#5{{#1}\fap {#2}\fa  {#3}\fa  {#4}\fa  {#5}}
\def\fPAAP #1#2#3#4#5{{#1}\fap {#2}\fa  {#3}\fa  {#4}\fap {#5}}
\def\fPAPA #1#2#3#4#5{{#1}\fap {#2}\fa  {#3}\fap {#4}\fa  {#5}}
\def\fPAPP #1#2#3#4#5{{#1}\fap {#2}\fa  {#3}\fap {#4}\fap {#5}}
\def\fPPAA #1#2#3#4#5{{#1}\fap {#2}\fap {#3}\fa  {#4}\fa  {#5}}
\def\fPPAP #1#2#3#4#5{{#1}\fap {#2}\fap {#3}\fa  {#4}\fap {#5}}
\def\fPPPA #1#2#3#4#5{{#1}\fap {#2}\fap {#3}\fap {#4}\fa  {#5}}
\def\fPPPP #1#2#3#4#5{{#1}\fap {#2}\fap {#3}\fap {#4}\fap {#5}}

\def\ttmlkeyword#1{\mathsfbf{#1}}% type theory metalanguage keyword
\def\ttolkeyword#1{\mathsfdc{#1}}% type theory objectlanguage keyword
\def\spreadmatrixcr{\spreadmatrixlines{3pt}}

\def\dataorsymbol{\pmbb{\mid}}
\def\dataforms#1= #2{\ttmlkeyword{data}\ {#1}\ \;\mathtt{=}\;\ {\def\dataor{\;\dataorsymbol\;}#2}}

\def\dataPRIME#1\\#2\enddataPRIME{%
...
\def\data#1\enddata{$$\spreadmatrixcr\dataPRIME#1\enddataPRIME$$}
\def\dataed#1{\dataPRIME #1\enddataPRIME}
\def\datased#1{&\dataPRIME #1\enddataPRIME}
\def\datas#1 #2\enddatas{$$
\let\dataed=\datased
\spreadmatrixcr
\xalignat#1\relax\fixmcount
#2
...
\def\ifthenelseed#1#2#3{% there is no non-(*ed) version
\ttolkeyword{if}~{#1}~\ttolkeyword{then}~{#2}~\ttolkeyword{else}~{#3}%
}
\def\caseofPRIMEsuminpattern   #1.#2-> #3\\{{\sumin #1.#2} & {\mapsto} &{#3}\\}
\def\caseofPRIMEgenericpattern #1->    #2\\{{#1}           & {\mapsto} &{#2}\\}
\def\caseofed#1#2{% there is no non-(*ed) version
\let\ca=\caseofPRIMEsuminpattern
\let\ga=\caseofPRIMEgenericpattern
\ttolkeyword{case}~{#1}~\ttolkeyword{of}~
    \left\{
    \matrix
        \format \l&\;\c\;&\l \\
        #2
...
% \funcoleqs is the only factored out part for fundef* and funio* macros
% ended up with code repetition for easier decoupling later on
\def\funcoleqs#1{% columnized functional equations
\matrix\format\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\ &{}\l\\
#1
\endmatrix
}

% fundef*: for defining functions together with their type
\def\fundefPRIME #1:#2\\#3\endfundefPRIME{%
\mathcoled {%
&#1\hastype #2 \\
&\!\funcoleqs{\vphantom{E^{E^E}}#3}
}}
\def\fundefed#1{\fundefPRIME #1\endfundefPRIME}
\def\fundefsed#1{&\fundefPRIME #1\endfundefPRIME}% repetition of code necessary here
\def\fundef#1\endfundef{$$\spreadmatrixcr\fundefPRIME #1\endfundefPRIME$$}
\def\fundefs#1 #2\endfundefs{$$
\let\fundefed=\fundefsed
\spreadmatrixcr
\xalignat#1\relax\fixmcount #2\endxalignat\saveeqnumber
$$}

% funio*: for listing input-output equations without the type on top
\def\funioPRIME#1\endfunioPRIME{\funcoleqs{#1}}
\def\funioed#1{\funioPRIME #1\endfunioPRIME}
\def\funiosed#1{&\funioPRIME #1\endfunioPRIME}
\def\funio#1\endfunio{$$\spreadmatrixcr\funioPRIME #1\endfunioPRIME$$}
\def\funios#1 #2\endfunios{$$
\let\funioed=\funiosed
\spreadmatrixcr
\xalignat#1\relax\fixmcount #2\endxalignat\saveeqnumber
$$}
...
%%{{{ category theory 
\let\thecat=\namedcat                  % \thecat {Set}             𝐒𝐞𝐭
\def\somecat#1{{\bbb #1}}              % \somecat C                ℂ
\let\cat=\somecat                      % \cat C                    ℂ
\def\catby#1{\cat C[#1]}               % \catby S                  ℂ[S]
\def\numcat#1{{\bbold #1}}             % \numcat 1                 𝟙
\def\one{{\mathit 1}}                  % \one                      1
\def\oneof#1{{\one_{#1}}}              % \oneof A                  1_A
\def\arrset#1#2#3{{#1}(#2,#3)}         % \arrset {\cat C} A B      ℂ(A,B)
\def\homset#1#2{\Hom(#1,#2)}           % \homset A B               Hom(A,B)
\def\homsetin#1#2#3{\Hom_{#1}(#2,#3)}  % \homsetin {\cat C} A B    Hom_C(A,B)
\def\prosetal#1#2{#1_{#2}}             % \prosetal \ints \leq      ℤ_(≤)
\def\theprosetalcat#1#2{\prosetal{\thecat{#1}}{(#2)}} % \theprosetalcat Int \leq  ℤ_(≤)
\DefOpP Obj
\DefOpP Arr
\DefOp  Hom
\DefOp  src
...
\let\dual=\dualposet
\def\inf{\knuthinf\nolimits} % inf is sameline by default
\def\sup{\knuthsup\nolimits} % sup is sameline by default
\DefOp glb
\DefOp lub
\DefOp lbs
\DefOp ubs
...
\def\minb{\mathord{\min_2}} % binary min
\def\maxb{\mathord{\max_2}} % binary max
%%}}}

%%{{{ group theory 
\def\go{\mathbin{\pmb{\cdot}}}
\def\gopp#1{{#1}^{\mathrmtiny{opp}}}
\def\goppp#1{\gopp{\paren{#1}}}
...
\def\dhole{{\mathord{-}}}% dash hole
\def\bhole{{\mathord{\color{holecolor}\bullet}}}% bullet hole
\def\ushole{{\mathord{\vphantom x\_\mkern-4mu\_}}}% underscore uhole
\def\vrhole{{\mathord{\vphantom x{\vrule height 0.0pt depth 0.4pt width 0.4em}}}}% vrule uhole
\let\uhole=\vrhole
\def\phole{{\mathord{\vphantom0\_}}}% placeholder or pattern-hole
\def\chole{{\ensuremath{\colorbox{holecolor}{$\vphantom 1\uhole$}}}}% colored hole
...
\def\dotswith#1{\dots#1\dots}
\def\holewith#1{\lthole#1\lthole}
\let\hole=\chole
\let\holed=\choled
%%}}}
...
\halign{%
\hfil##&##\hfil\cr
\proofcasestylize{\algoinputname:}  & #1\cr
\proofcasestylize{\algooutputname:} & #2\cr
}
}
...
\def\limto{\mathrel{\to}}
\def\limfrom{\mathrel{\from}}
\let\tendsto=\limto
\let\tendsfrom=\limfrom
\let\tends=\tendsto
\def\nlimto{\smartnot{\limto}}
\def\nlimfrom{\smartnot{\limfrom}}
\let\ntendsto=\nlimto
\let\ntendsfrom=\nlimfrom
\let\ntends=\ntendsto
\def\uniformlyto{\mathrel{\rrarr}}
\def\uniformlyfrom{\mathrel{\llarr}}
\let\unito=\uniformlyto
\let\unifrom=\uniformlyfrom
\let\utendsto=\uniformlyto
\let\utendsfrom=\uniformlyfrom
\let\utends=\utendsto
...
{\replheadstyle\sayName{proof}}\strut\cr
\replhrule\cr
#1%
}
}%
...
{\replheadstyle\sayName{givens}}\strut\cr
\replhrule\cr
#1%
}
}%
...
{\replheadstyle\sayName{goals}}\strut\cr
\replhrule\cr
#1%
}
}%
...
\compute .. \endcompute
Computation block, as a specialized \mathcols 2 which supports \by:
\compute
blah
&= blah'  \by {why} \\
&= blah'' \by {why'} \\
...
\funcompute .. \endfuncompute
Wim Feijen style of writing computations, popular in functional programming:
\funcompute
blah
  \does =    \by {why} \\
blah'
...
One can have computations in parallel with:
\funcomputes 3
\funcomputed { ... } & \funcomputed { ... } & \funcomputed { ... }
\endfuncomputes
