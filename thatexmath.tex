...
% \DefTHING #1 #2 \defs a command #1 to be printed as #2 and also take cares
% the definition of the "parenthesized" version of it if it makes sense
...
\def\abbrdefiff{\;\mathrel{{\buildrel{\text{\sixss def}} \over {\Lleftarrow\mkern-3mu\Rrightarrow}}\;}}
\def\intiff{\;\mathrel{\Lleftarrow\mkern-3mu\Rrightarrow}\;}
\let\sugdefiff=\abbrdefiff
\let\sugariff=\intiff
\let\abbriff=\intiff
...
\def\mapstoarrow#1{\mathrel{\decorateover{#1}{\longmapsto}}}
...
\def\lappp#1{\lapp\paren{#1}}% parenthesized lapp
\def\fappp#1{\fapp\paren{#1}}% parenthesized fapp
\let\fa=\fapp
\let\fap=\fappp
\let\la=\lapp
\let\lap=\lappp
...
\let\normal=\normalsubgroup
\let\normsub=\normalsubgroup
\def\cosetsL#1{{\cal #1}_{\mathrmsmall L}}
\def\cosetsR#1{{\cal #1}_{\mathrmsmall R}}
\def\congL{\mathrel{\cong_{\mathrmsmall L}}}
\def\congR{\mathrel{\cong_{\mathrmsmall R}}}
\DefOp Hom
\DefOp End
\DefOp Aut
\DefOp Inn
\DefOp Out
\def\End{\namedop{End}}
\def\Endp#1{\End\funparen{#1}}
\def\Aut{\namedop{Aut}}
\def\Autp#1{\Aut\funparen{#1}}
\def\Inn{\namedop{Inn}}
\def\Innp#1{\Inn\funparen{#1}}
\def\Out{\namedop{Out}}
\def\Outp#1{\Out\funparen{#1}}
