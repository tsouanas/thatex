...
\def\stepproof#1\endstepproof{$$\alignat2\relax\fixmcount #1\endalignat\saveeqnumber$$}
...
\def\proofsteptby#1#2{&\text{#1}\bynb{#2}\\}
\def\proofstepmby#1#2{&{#1}\bynb{#2}\\}
...
\DefStyle replR {\color{alertR}}
\DefStyle replG {\color{alertG}}
\DefStyle replB {\color{alertB}}

\def\replalertstyle{\ifreplignorealerts\else\color{alert}\fi}
