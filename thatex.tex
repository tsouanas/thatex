...
\def\frameit#1#2#3{\hbox{\vrule width #1\vtop{\vbox{\hrule height #1\kern#2\hbox{\kern#2#3\kern#2}}\kern#2\hrule height #1}\vrule width #2\relax}}
\let\footnote=\numberedfootnote
\def\foot#1\endfoot{\footnote{#1} }% XXX: ugly
...
% unordered lists
\def\begul #1\endul {\unorderedlist{#1}\endunorderedlist\noindent}

% ordered lists: TODO work on this \fmt thing
\def\begol #1\endol {%
    \orderedlist
    {\def\li{\eplainli}
     \def\fmt ##1\m##2 {\def\stylizedmarker{{##1}\marker{##2}}}
     #1}
    \endorderedlist\noindent
}
\def\begoL #1: #2\endoL {% (L)abelled: #1: prefix; #2: body (items)
\begingroup
\def\Li :##1 {\li\definexref{#1:##1}{(\marker)}{li}}
\begol
#2
\endol
\endgroup\noindent
...
\def\begiL #1: #2\endiL {%
\begingroup
\def\Li ##1 :##2 {\li ##1 \definexref{#1:##2}{(##1)}{li}}
\begil
#2
\endil
\endgroup\noindent
...
\def\dialogue#1\enddialogue{{\begil\def\say{\item{--}}\def\who##1{\item{##1:}}#1\endil}}
