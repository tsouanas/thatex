...
\def\glossor{{\;;\;}}
\line{\llap{##6}\quad\hbox{##7}\quad\dotfill{}\enspace{\refn{##5},\enspace{##4}}}%
}
...
%%{{{ \Report : better use a shell script for this 
\def\makereport{%
\Report
...
% \DefWord(s) define: #1name #1word (#1sname)
\def\DefWord #1 #2{% #1: handle ; #2: singular
\expandafter\def\csname #1name\endcsname{#2}
\expandafter\def\csname #1word\endcsname{#2}
}
\def\DefWords #1 #2#3{% #2: singular ; #3 plural
\DefWord #1 {#2}
\expandafter\def\csname #1sname\endcsname{#3}
}
...
% sectiontotal makes no sense: it doesn't reset
\exercisetotal=0
\problemtotal=0
\chaptertotal=0
...
\def\chapintro#1\par{\noindent #1\endgraf\vskip\chaptertitleskip}

\def\endchapter{%
\ifdim\pagetotal=0pt\relax\else\vfill$$\woohoo$$\eject\fi%
}

\def\chapterblah#1. #2\par{%
\titlepage%
\topmarker={}%
\toptitle={#1}%
\noteno=0%
...
\def\secintro#1\par{\medskip\noindent #1\endgraf\vskip\sectiontitleskip}
...
\def\endsectionoid{%
\vbox{}\vskip 0pt\relax
\par%
\vcur=0pt
\advance \vcur by \pagetotal
\advance \vcur by \vminsec
\ifdim\vcur>\vsize% not enough space to start section(oid)
\eject%
\else% enough space to start section(oid)
\vskip 8ex\relax\goodbreak%
\fi
}
...
%%%%
% preproof 
\long\def\preproof####1. ####2\endpreproof{%
\medskip
\endgraf%
\bgroup\rm%
\ifEqStr{####1}{}%
\else%
\noindent\argumentationheadstyle{####1}. %
\fi%
\noindent ####2%
\egroup%
\endgraf%
...
\def\question#1. #2\par{\genproclaim{question}{#1}{#2}{Q}}
\def\axiom#1. #2\par{\genproclaim{axiom}{#1}{#2}{A}}
\def\law#1. #2\par{\genproclaim{law}{#1}{#2}{}}
\def\principle#1. #2\par{\genproclaim{principle}{#1}{#2}{}}
\def\hypothesis#1. #2\par{\genproclaim{hypothesis}{#1}{#2}{}}
...
\def\advice#1. #2\par{\gennote{advice}{#1}{#2}{}}
\def\beware#1. #2\par{\gennote{beware}{#1}{#2}{}}
\def\warning#1. #2\par{\gennote{warning}{#1}{#2}{}}
\def\primitive#1. #2\par{\gennote{primitive}{#1}{#2}{P}}
\def\definition#1. #2\par{\gennote{definition}{#1}{#2}{D}}
\def\notation#1. #2\par{\gennote{notation}{#1}{#2}{}}
\def\pseudodefinition#1. #2\par{\gennote{pseudodefinition}{#1}{#2}{D}}
\def\specification#1. #2\par{\gennote{specification}{#1}{#2}{S}}
\def\implementation#1. #2\par{\gennote{implementation}{#1}{#2}{D}}
\def\teaser#1. #2\par{\gennote{teaser}{#1}{#2}{}}
\def\joke#1. #2\par{\genproclaim{joke}{#1}{#2}{}}
...
\def\iipl#1 {\ii{#1}}% to be redefined in thatexbook-LANG
\def\nameindex#1#2{%
\sndxname{#1}{#2}%
}
%%}}}
...
#2%
\XXXeolfill$\qexsymbol$\par\egroup\medbreak
}% /example-scope
}
%%}}}
...
\def\sourcecode#1; {\endgraf\bigskip{\def\setuplistinghook{\linenumberedlisting\nolastlinelisting}\listing{code/#1}}\bigskip\endgraf\noindent}
%%}}}
