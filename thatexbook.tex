...
\noindent{\bf \reftagofid[##2]\fullprooftagdecorator.}
...
\ifnum\number\i=##2\relax
\noindent
{\bf {\reftagofid[##1]\hinttagdecorator##2}.~}%
...
% XXX: ^ to hintentry htan mesa sto Repeat
\Repeat \for{i} \from{1} \to{\maxhintno} \do {% repeat
...
\ifnum##2>\curchapterno
    ...
    \goodbreak
    \centerline{\fourteenbf\sayName{chapter} \chapterthe{##2}}\solutionskip
\fi
\noindent
\begingroup
\def\paragid{##1}%
{\bf \reftagofid[##1]\solutiontagdecorator.\enspace}%
\autogetpdefs
##3%
\endgroup
\solutionskip
...
\everyfootnote={\baselineskip=12pt \sizeix\rm}
...
\Newskip spoiler=3\bigskipamount;
...
%%{{{ \structmixin 
% \structmixin #1 sets struct attributes to \#1attr
% CLASSclass should be set before calling structmixin
\def\structmixin#1{%
\edef\structbase{#1}%
...
\def\structparen{}%
\def\structlabel{}%
\def\structname{\sayName{\structclass}}%
\def\structfull{\structformal\ifvanishes\structtitle\\\else\spacechar(\structtitle)\fi}%
\def\structsmart{\structname \structtag\ifvanishes\structparen\\\else\spacechar(\structparen)\fi}%
\Mixin
    class tag
    id formal title aka name
    full smart
    \from {\structbase}
    \to struct
% class should be set before calling structmixin:
\ifvanishes\structclass\\\thatexerror[class must be set before you structmixin]\fi
}
%%}}}
...
\WhenStrNempty {\structlabel} {% if we have \structlabel
...
\definecolor{introcolor}{cmyk}{0.5,.1,.3,.4}
...
%%%% tag and id are needed to call structmixin
...
%%%% id is needed to call structmixin
\edef\secid      {sec\number\secidno}%
...
\par
\vcur=0pt\relax
\advance \vcur by \pagetotal\relax
\advance \vcur by \vminsec\relax
\ifdim \vcur > \vsize % not enough space to start section(oid)
    \vfill
    \eject
\else % enough space to start section(oid)
    \secskip
    \goodbreak
\fi
\secleave
...
%%%% structmixin (id and class needed)
\structmixin{parag}%
%%%% counters
\resetparagwidecounters
...
\(blah\|note\|question\|remark\|warning\|beware\|advice\|primitive\|definition\|pseudodefinition\|grammar\|algorithm\|notation\|specification\|teaser\|joke\|implementation\|answer\|proof\|refutation\)
\(example\|nonexample\)
\(theorem\|lemma\|property\|criterion\|corollary\|proposition\)
\(conjecture\|axiom\|hypothesis\|principle\|law\)
\(exercise\|problem\|codeit\|openproblem\)

\(blah\|note\|question\|remark\|warning\|beware\|advice\|primitive\|definition\|pseudodefinition\|grammar\|algorithm\|notation\|specification\|teaser\|joke\|implementation\|answer\|proof\|refutation\|example\|nonexample\|theorem\|lemma\|property\|criterion\|corollary\|proposition\|conjecture\|axiom\|hypothesis\|principle\|law\|exercise\|problem\|codeit\|openproblem\)
